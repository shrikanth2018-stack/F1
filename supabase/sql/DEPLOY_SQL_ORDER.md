...
> ## ⚠ THIS FILE IS THE CHANGELOG. IT IS NOT THE SCHEMA.
>
> **What is actually deployed lives in `supabase/schema/live_schema.sql`** — a
> `pg_dump` of production, regenerated by `npm run schema:snapshot`, plus
> `live_jobs.txt` for the cron jobs and extensions the dump excludes.
>
> Two reasons that distinction matters:
>
> 1. **This directory cannot rebuild the database.** On 17 Aug 2026
>    `schema.sql` held 43 tables and 51 functions; production had 54 and 105.
>    The 141 files here are the history of how it was built, not a description
>    of what exists.
> 2. **Twenty-five functions are defined in two, three or four files each, and
>    the last one applied silently wins.** `assign_hub_operator` is in four —
>    re-running the oldest re-opens a privilege-escalation hole closed in §36.
>    Before re-applying any file, read the deployed body in the snapshot.
>
> After applying anything: `npm run schema:snapshot` and commit
> `supabase/schema/` in the same sitting. `npm run schema:check` fails when the
> repo and production disagree, and a nightly GitHub Action runs it once two
